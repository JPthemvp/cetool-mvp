...
import type { AttackSurface } from "@/lib/attack-surface";
import { CLAUSE_BY_ID } from "@/lib/ce-framework";

// ── Attack Surface Card (Shodan / Censys) ─────────────────────────────────────

const CVSS_TONE = (cvss: number) =>
  cvss >= 9 ? "text-red-400" : cvss >= 7 ? "text-orange-400" : cvss >= 4 ? "text-amber-400" : "text-brand-100/70";

function AttackSurfaceCard({ data }: { data: AttackSurface }) {
  const [open, setOpen] = useState(true);
  if (data.hosts.length === 0) return null;

  const allPorts = data.hosts.flatMap((h) => {
    const shodanPorts = h.shodan?.ports ?? [];
    const censysPorts = (h.censys?.services ?? []).map((s) => ({
      port: s.port,
      protocol: s.transport.toLowerCase(),
      service: s.serviceName,
      product: undefined as string | undefined,
      version: undefined as string | undefined,
    }));
    // Merge: prefer Shodan detail, fill with Censys for ports Shodan didn't see
    const seen = new Set(shodanPorts.map((p) => p.port));
    return [...shodanPorts, ...censysPorts.filter((p) => !seen.has(p.port))].map((p) => ({
      ...p,
      ip: h.ip,
    }));
  });

  const allVulns = data.hosts.flatMap((h) =>
    (h.shodan?.vulns ?? []).map((v) => ({ ...v, ip: h.ip }))
  ).sort((a, b) => b.cvss - a.cvss);

  const hostInfo = data.hosts[0]?.shodan;
  ...
  return (
    <Card className="mt-6 overflow-hidden">
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left transition hover:bg-ink-850/50"
      >
        <div className="flex items-center gap-3">
          <span className="text-[15px] font-semibold text-white">Attack Surface</span>
          <Pill tone={allVulns.length > 0 ? "bad" : "info"}>
            {allVulns.length > 0 ? `${allVulns.length} CVE${allVulns.length > 1 ? "s" : ""} found` : `${allPorts.length} open ports`}
          </Pill>
        </div>
        <span className="text-[11px] text-brand-200/60">{open ? "▲ Collapse" : "▼ Expand"}</span>
      </button>
      ...
      {open && (
        <div className="border-t border-brand-700/30">
          {/* Host info strip */}
          {hostInfo && (
            <div className="flex flex-wrap gap-x-6 gap-y-1 border-b border-brand-700/20 bg-ink-950/40 px-5 py-3 text-[12px] text-brand-100/70">
              {hostInfo.org && <span><span className="text-brand-200/50">Org </span>{hostInfo.org}</span>}
              {hostInfo.isp && <span><span className="text-brand-200/50">ISP </span>{hostInfo.isp}</span>}
              {hostInfo.country && <span><span className="text-brand-200/50">Country </span>{hostInfo.country}{hostInfo.city ? `, ${hostInfo.city}` : ""}</span>}
              {hostInfo.os && <span><span className="text-brand-200/50">OS </span>{hostInfo.os}</span>}
              {hostInfo.lastSeen && <span><span className="text-brand-200/50">Last seen </span>{new Date(hostInfo.lastSeen).toLocaleDateString("en-SG")}</span>}
            </div>
          )}

          {/* CVEs */}
          {allVulns.length > 0 && (
            <div className="border-b border-brand-700/30 px-5 py-4">
              <p className="mb-3 text-[11px] font-semibold uppercase tracking-wide text-csa-300">
                Known Vulnerabilities (CVEs)
              </p>
              <div className="space-y-2">
                {allVulns.map((v) => (
                  <div key={`${v.ip}-${v.cve}`} className="flex items-start gap-3">
                    <span className={`shrink-0 font-mono text-[11px] font-semibold tabular-nums ${CVSS_TONE(v.cvss)}`}>
                      {v.cvss.toFixed(1)}
                    </span>
                    <div className="min-w-0">
                      <a
                        ...
                        className="font-mono text-[12px] text-brand-300 underline-offset-2 hover:underline"
                      >
                        {v.cve}
                      </a>
                      {v.summary && (
                        <p className="mt-0.5 text-[12px] leading-snug text-brand-100/70">{v.summary}</p>
                      )}
                      <p className="text-[11px] text-brand-200/50">on {v.ip}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="px-5 py-4">
              ...
              <div className="overflow-x-auto scroll-thin">
                <table className="w-full min-w-[480px] text-left text-[12px]">
                  <thead>
                    <tr className="border-b border-ink-700 text-[11px] uppercase tracking-wide text-brand-100/50">
                      <th className="pb-2 pr-4 font-medium">Port</th>
                      <th className="pb-2 pr-4 font-medium">Protocol</th>
                      <th className="pb-2 pr-4 font-medium">Service</th>
                      <th className="pb-2 pr-4 font-medium">Product / Version</th>
                  ...
                  <tbody>
                    {allPorts.map((p, i) => (
                      <tr key={i} className="border-b border-ink-800/40 last:border-0">
                        <td className="py-2 pr-4 font-mono font-semibold tabular-nums text-white">{p.port}</td>
                        <td className="py-2 pr-4 text-brand-100/70">{p.protocol.toUpperCase()}</td>
                        <td className="py-2 pr-4 text-brand-100/80">{p.service ?? "—"}</td>
                        <td className="py-2 pr-4 text-brand-100/60">
                          {[p.product, p.version].filter(Boolean).join(" ") || "—"}
                        </td>
                        <td className="py-2 font-mono text-[11px] text-brand-200/60">{p.ip}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {/* Attribution */}
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-brand-700/20 bg-ink-950/40 px-5 py-3">
            <span className="text-[11px] text-brand-200/50">Data sourced from</span>
            {data.sources.map((src) => (
              <a
                key={src}
                href={src === "Shodan" ? "https://www.shodan.io" : "https://search.censys.io"}
                target="_blank"
                rel="noreferrer noopener"
                className="text-[11px] font-medium text-brand-300 underline-offset-2 hover:underline"
              >
                {src}
              </a>
            ))}
            <span className="text-[11px] text-brand-200/40">
              · Passive lookup only — no probing of the target
              · Queried {new Date(data.queriedAt).toLocaleString("en-SG")}
            </span>
          </div>
        </div>
      )}
    ...
    <div>
      ...
      {scan && scan.reachable && (
        <>
          ...
          {/* ── Attack surface (Shodan / Censys) ─────────────────────── */}
          {scan.attackSurface && (
            <AttackSurfaceCard data={scan.attackSurface} />
          )}
        </>
      )}
    </div>
